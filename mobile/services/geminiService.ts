...
export async function chatAboutCar(message: string, history: {role: string, text: string}[] = []): Promise<string> {
  const model = "gemini-3-flash-preview";
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents,
      config: {
        systemInstruction: "You are a helpful car expert assistant for CarWise-AI. Answer questions about cars, specs, reliability, and maintenance. You can also help users understand which car is best for them. Be concise, friendly, and factual.",
        tools: [{ googleSearch: {} }],
      }
    });
